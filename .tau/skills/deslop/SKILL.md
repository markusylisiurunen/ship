...
- **Contract drift** across domain types, API or SDK types, wire formats, view models, prompts, and docs. Keep these shapes canonical and symmetric.
...
When compatibility versus simplicity is genuinely a judgment call, inspect the repository, issue, and pull request context for evidence that existing data or contracts must be preserved. Ask the user only when the workflow allows interaction and the answer cannot be established from the available context.
...
Migrations that rewrite old payload shapes for a feature that never shipped. Dual-read code paths guarding against nonexistent historical data. Service A defensively coercing responses from Service B when the services always ship together. Verify release and compatibility requirements from the repository and task context; when no historical data or external client exists, drop the shim and tighten the schema.
...
Before reporting done, verify the touched code end-to-end. The relevant `AGENTS.md` files define the commands (build, lint, format, tests) and conventions (line lengths, tooling, style) for each app you touched. Read them and follow them. Run formatting first when required, then the repository's checks, build, and appropriate tests. Do not run interactive applications or inspect dependencies unless explicitly asked.
